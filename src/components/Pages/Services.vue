<template>
  <v-container>
    <v-layout>
      <v-flex>
        <h2 class="headline primary--text pb-3">Доступность веб-сервисов</h2>
          ...
          <v-card>
            ...
            <v-card-title>
              ...
              <v-text-field
                v-model="search"
                append-icon="search"
                label="Поиск по названию веб-сервиса"
                clearable
                single-line
                hide-details
              >
  ...
  export default {
    ...
    computed: {
      ...
      xlsExport() {
        ...
        return `http://zabbix.fm.epbs.ru/zabbix/reports/api/v1/services/export/xlsx/?api_token=${config.API_TOKEN}&period=${period}`
      }
    },
    filters: {
      namedMonth(value) {
    ...
    created() {
      ...
      !this.dev && this.$store.dispatch('setServices', {
        from: this.from,
        to: this.to
      })
    }
