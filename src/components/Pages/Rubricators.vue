<template>
  <v-container>
    <v-layout>
      <v-flex>
        <h2 class="headline primary--text pb-3">Доступность рубрикаторов</h2>
          ...
          <v-card>
            ...
            <v-card-title>
              ...
              <v-text-field
                v-model="search"
                append-icon="search"
                label="Поиск по названию рубрикатора"
                clearable
                single-line
                hide-details
              >
...
<script>
  ...
  export default {
    ...
    computed: {
      tableData() {
        return this.$store.getters.rubricators
      },
      tableHeaders() {
      ...
      xlsExport() {
        ...
        return `http://zabbix.fm.epbs.ru/zabbix/reports/api/v1/rubricators/export/xlsx/?api_token=${config.API_TOKEN}&period=${period}`
      }
    },
    filters: {
      namedMonth(value) {
    ...
    methods: {
      getData() {
        return this.dev
          ? mockTableData
          : this.$store.dispatch('setRubricators', {
    ...
    created() {
      ...
      !this.dev && this.$store.dispatch('setRubricators', {
        from: this.from,
        to: this.to
      })
    }
