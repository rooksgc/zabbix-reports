<template>
  <v-container>
    <v-layout>
      <v-flex>
        <h2 class="headline primary--text pb-3">Срабатываение логических контролей</h2>
        ...
        <template v-if="tableData">
          ...
          <v-btn
            color="deep-purple lighten-1"
            class="white--text mb-4 ml-0 mt-4"
            :disabled="loading"
            @click="fetchData"
          >
            <v-icon left dark>swap_vert</v-icon>
            <span>Загрузить в базу данные за сегодня</span>
          </v-btn>
...
<script>
  ...
  export default {
    ...
    computed: {
      tableData() {
        return this.dev ? this.getData() : this.$store.getters.logicalControls
      },
      tableHeaders() {
      ...
      xlsExport() {
        ...
        return `http://zabbix.fm.epbs.ru/zabbix/reports/api/v1/logical-controls/export/xlsx/?api_token=${config.API_TOKEN}&period=${period}`
      }
    },
    filters: {
      namedMonth(value) {
    ...
    methods: {
      getData() {
        return this.dev
          ? mockTableData
          : this.$store.dispatch('setLogicalControls', {
            from: this.$store.getters.from,
            to: this.$store.getters.to
          })
      },
      fetchData() {
        this.$store.dispatch('fetchLogicalControlsToday')
      },
      setMonth(month) {
    ...
    created() {
      ...
      !this.dev && this.$store.dispatch('setLogicalControls', {
        from: this.from,
        to: this.to
      })
    }
